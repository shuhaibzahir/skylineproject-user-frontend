 ...
 // the menu options
  ...
  const handleClose = () => {
    ...
    const deleteComment = (data) => {
      ...
      axios.put(`https://skyline.shuhaib.host/server/api/delete/comment/${postData._id}/${data._id}`,{},  {
        headers: {
  ...
  const unlike = () => {
    axios
      .put(
        `https://skyline.shuhaib.host/server/api/post/dislike/${singlePost._id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${decrypedUserDetails.token}`,
  ...
  const likePost = () => {
    axios
      .put(
        `https://skyline.shuhaib.host/server/api/post/like/${singlePost._id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${decrypedUserDetails.token}`,
  ...
  const addCommentToPost = (postData) => {
    ...
    if (userComment.replaceAll(" ", "").length > 1) {
      axios
        .put(
          `https://skyline.shuhaib.host/server/api/post/add/comment/${postData._id}`,
          { comment: userComment },
          {
            headers: {
              Authorization: `Bearer ${decrypedUserDetails.token}`,
